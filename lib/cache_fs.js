...
const fs_json_1 = require("./fs_json");
...
async function loadCacheRaw(fileName) {
    ...
    try {
        await (0, promises_1.access)(Dir(fileName));
    }
    catch (e) {
        return null;
    }
    return await (0, fs_json_1.readJson)(Dir(fileName));
}
...
async function writeCache(fileName, data) {
    await ensureCacheFolder();
    await (0, fs_json_1.writeJson)(Dir(fileName), { data, timestamp: Date.now() });
}

...
async function reloadGlobalEmotes() {
    const { data: emotes, ...rate } = await (0, api_1.globalEmotes)();
...
async function reloadChannel(channel) {
    ...
    const { data: emotes, ...rateEmotes } = await (0, api_1.channelEmotes)(channel);
    const { data: identifier, ...rateIdentifier } = await (0, api_1.channelIdentifier)(channel);

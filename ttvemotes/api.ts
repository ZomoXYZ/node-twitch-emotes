 ...
 */
export function handleProxyResponse(resp: Response): ApiResponse<string|null> {
...
export const channelIdentifier = (channel: string): Promise<ApiResponse<ChannelIdentifier>> =>
        fetch(`https://emotes.adamcy.pl/v1/channel/${isChannelThrow(channel)}/id`).then(res => handleResponse<ChannelIdentifier>(res));
...
export const globalEmotes = (services: Services = 'all'): Promise<ApiResponse<EmoteData[]>> =>
        fetch(`https://emotes.adamcy.pl/v1/global/emotes/${correctServices(services)}`).then(res => handleResponse<EmoteData[]>(res));
 ...
 * Math pattern `/^[^\.]+(all|(\.?twitch|\.?7tv|\.?bttv|\.?ffz)+)$/`
 */
export const channelEmotes = (channel: string, services: Services = 'all'): Promise<ApiResponse<EmoteData[]>> =>
        fetch(`https://emotes.adamcy.pl/v1/channel/${isChannelThrow(channel)}/emotes/${correctServices(services)}`).then(res => handleResponse<EmoteData[]>(res));
 ...
 * @param services Possible values: all or any combination of: twitch, 7tv, bttv, ffz combined using dots (e.g. twitch.7tv)
 */
export const proxyChannelEmote = (channel: string, services: Services = 'all'): Promise<ApiResponse<string | null>> =>
        fetch(`https://emotes.adamcy.pl/v1/channel/${isChannelThrow(channel)}/emotes/${correctServices(services)}/proxy`).then(res => handleProxyResponse(res));